...
const LeagueManagement: React.FC = () => {
  ...
  // Auto-sync player's connected leagues on component mount
  useEffect(() => {
    if (managerData?.id) {
      syncPlayerLeagues();
    }
  ...
  const syncPlayerLeagues = async () => {
    ...
    try {
      ...
      if (leaguesWithStandings.length > 0) {
        setSelectedLeague(leaguesWithStandings[0]);
      }
    } catch (err) {
      console.error('Failed to sync player leagues:', err);
  ...
  const catchUpCalculator = useMemo(() => {
    if (!selectedLeague || !selectedLeague.standings) return null;
    
    const userStanding = selectedLeague.standings.find((s: any) => s.entry_id === managerData?.id);
    if (!userStanding) return null;

    return selectedLeague.standings.map((standing: any) => ({
      ...standing,
      pointsBehind: Math.max(0, standing.total - userStanding.total),
      positionsAhead: Math.max(0, userStanding.rank - standing.rank),
    })).sort((a: any, b: any) => b.pointsBehind - a.pointsBehind);
  }, [selectedLeague, managerData]);
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
            ...
            <div className="space-y-3">
              <input
                ...
                onKeyPress={(e) => e.key === 'Enter' && handleImportLeague()}
                disabled={loading}
                className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 rounded-lg focus:ring-2 focus:ring-purple-500 outline-none disabled:opacity-50"
              />
              <button
                onClick={handleImportLeague}
                disabled={loading}
                className="w-full px-4 py-2 bg-purple-600 hover:bg-purple-700 disabled:bg-gray-400 text-white rounded-lg font-semibold transition"
              >
                {loading ? 'Loading...' : 'Import League'}
              </button>
            </div>
            ...
            <div className="space-y-6">
              ...
              {catchUpCalculator && (
                <div className="bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-800 rounded-lg p-6">
                  <h3 className="text-lg font-bold text-blue-900 dark:text-blue-300 mb-4">Catch-up Calculator</h3>
                  <p className="text-sm text-blue-800 dark:text-blue-200 mb-4">
                    Points and positions needed to catch up with other managers in this league.
                  </p>
                  <div className="space-y-2 max-h-[400px] overflow-y-auto">
                    {catchUpCalculator.slice(0, 10).map((standing: any, idx: number) => (
                      <div key={idx} className="flex items-center justify-between p-3 bg-white dark:bg-gray-800 rounded">
                        <div className="flex-1">
                          <div className="font-semibold text-sm text-gray-800 dark:text-gray-200">#{standing.rank || '-'}</div>
                          <div className="text-xs text-gray-600 dark:text-gray-400">Manager Name</div>
                        </div>
                        <div className="text-right">
                          {standing.pointsBehind > 0 && (
                            <div className="text-sm font-bold text-red-600 dark:text-red-400">
                              -{standing.pointsBehind} pts
                            </div>
                          )}
                          {standing.positionsAhead > 0 && (
                            <div className="text-xs text-gray-600 dark:text-gray-400">
                              ↑ {standing.positionsAhead} positions
                            </div>
                          )}
                          {standing.pointsBehind === 0 && standing.positionsAhead === 0 && (
              ...
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
                ...
                <div className="text-center py-8 text-gray-500 dark:text-gray-400">
                  <p>League standings would appear here</p>
                  <p className="text-xs mt-2">Connect to FPL API to populate standings</p>
                </div>
              </div>
            </div>
          )}
